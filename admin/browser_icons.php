...
$tabs = array(
	1 => array($txtf->display("view_detail"), "browser.php"."?mode=" . $mode . "&filter=" . urlencode($filter) . "&folder=" . urlencode($folder)."&sort=$sort&sort_type=$sort_type"),
	2 => array($txtf->display("view_icon"), "browser_icons.php"."?mode=" . $mode . "&filter=" . urlencode($filter) . "&folder=" . urlencode($folder)."&sort=$sort&sort_type=$sort_type")
);
...
function doFile ($nr, $id, $obj, $text, $type, $date, $folder) {
	...
	$tpl->addDataItem("ROWS.URL$nr", "javascript:objInfo('$id', '$obj', '$type', '$date', '$size', '" . SITE_URL . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "." . $type . "', '$url1','$url2','".urlencode($folder)."');");
	if ($text == "") $text = "&nbsp;";
