...
function doFile ($nr, $id, $obj, $text, $type, $date, $folder) {
	...
	if ($type == "gif" || $type == "jpg" || $type == "png" || $type == "tif") {
		...
		if (file_exists(SITE_PATH . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "_thumb." . $type)) {
			$icon = "<img src=\"../".$GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "_thumb." . $type."\" align=absmiddle border=0 alt=\"$text\">";
		}
		else {
	...
	$tpl->addDataItem("ROWS.URL$nr", "javascript:selectImage('$id', '$obj', '$type', '$date', '$size', '" . SITE_URL . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "." . $type . "', '$url1','$url2');");
	if ($text == "") $text = "&nbsp;";
