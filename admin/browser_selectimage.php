...
$tpl->setTemplateFile("tmpl/browser_selectimage.html");
...
// ###############################################################
		...
		$tpl->addDataItem("TITLE", $txtf->display("info_choose_pic") . ": " . $GLOBALS["directory"]["upload"] . "/"  . $folder);
		...
		$tabs = array(
			1 => array($txtf->display("view_detail"), "browser_selectimage.php"."?mode=" . $mode . "&filter=" . urlencode($filter) . "&folder=" . urlencode($folder)."&sort=$sort&sort_type=$sort_type"),
			2 => array($txtf->display("view_icon"), "browser_icons_selectimage.php"."?mode=" . $mode . "&filter=" . urlencode($filter) . "&folder=" . urlencode($folder)."&sort=$sort&sort_type=$sort_type"),
		...
		function doFile ($id, $obj, $text, $type, $date, $folder) {
			...
			if ($type == "gif" || $type == "jpg" || $type == "png") {
			    if (false !== strpos($obj, '_thumb')) {
					$url1 = SITE_URL . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "." . $type;
				}
				else {
				if (file_exists(SITE_PATH . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "_thumb." . $type)) {
					$url1 = SITE_URL . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "_thumb." . $type;
				}
				else {
					$url1 = "";
				}
			...
			$tpl->addDataItem("ROWS.URL", "javascript:selectImage('$id', '$obj', '$type', '$date', '$size', '" . SITE_URL . "/" . $GLOBALS["directory"]["upload"] . "/"  . $folder . $obj . "." . $type . "', '$url1','$url2');");
			if ($text == "") $text = "&nbsp;";
			...
			if ($id) {
				$tpl->addDataItem("ROWS.DELURL", "files_admin.php?do=delete&id=".$id."&folder=".urlencode($folder));
			}
			else {
				$tpl->addDataItem("ROWS.DELURL", "files_delete.php?do=delete&file=".$url2."&folder=".urlencode($folder));
			}
