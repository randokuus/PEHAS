...
$table = "files_imagesizes"; // SQL table name to be administered
...
$fields = array(
    "size"  => $txtf->display("imagesize_size"),
    "name" => $txtf->display("imagesize_name")
);
...
function external() {
    ...
    $adm->assignProp("size", "extra", $txtf->display("imagesize_size_extra"));
...
if ($show == "add" || ($do == "add" && is_array($res))) {
    $tpl->addDataItem("TITLE", $txtf->display("imagesizes"));
    $active_tab = 1;
}
else {
    $tpl->addDataItem("TITLE", $txtf->display("imagesizes"));
