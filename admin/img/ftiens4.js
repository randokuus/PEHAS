...
{ 
  ...
  /*if (this.hreference) 
  { 
  docW = docW + '<a href="javascript:nodeClick()" ';
  if (isTextLink) docW += "id=\"itemTextLink"+this.id+"\" ";
  if (browserVersion > 0) docW = docW + "onClick='javascript:clickOnFolder(\""+this.getID()+"\")'"
  docW = docW + ">"
  } else {
	docW = docW + "<a>"
  }*/
  docW = (this.hreference) ? docW + '<a href="javascript:nodeClick(' + this.id + ')"' : "<a";
  //if (this.hreference) 
  //{ 
	/*if (USEFRAMES)
	...
	// docW = docW + '<a href="javascript:nodeClick(' + this.id + ')" ';
...
{ 
  ...
  var fullLink = "href=\"javascript:nodeClick("+this.id+")\"";
  this.isRendered = 1
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2node.gif'></td>"
    } 
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top><img src='" + ICONPATH + "ftv2vertline.gif'></td>"
		}
...
function findActive()
{
  ...
  for(i=0;i<nEntries;i++) {
	  if (indexOfEntries[i].stateActive) id = i;
  }
  if (id)
  	return indexOfEntries[id]
  else
...
function setItemLink(item, optionFlags, linkData) {
  ...
  item.hreference = linkData;
  item.target = target;
  item.typeid = optionFlags;
}
