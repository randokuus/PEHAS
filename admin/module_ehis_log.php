...
$txtf = new Text($language2, "module_isic_log");

// ##############################################################
// ##############################################################

$table = "module_isic_log"; // SQL table name to be administered
...
$required = array(
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
    ...
    $from = array(
        $table,
        "LEFT JOIN module_user_users ON $table.record_id = module_user_users.user"
    );

    $where = "module_name = 'ehis_query'";
...
function external() {
    global $adm, $show, $txtf, $txt, $table, $group, $language, $id, $structure, $card_id;
    //$sq = new sql;

    $adm->assignProp("event_type", "type", "select");
    $adm->assignProp("event_type", "list", array("4" => $txtf->display("success"), "5" => $txtf->display("error")));
    ...
    $adm->assignProp("record_id", "type", "select");
    $adm->assignExternal("record_id", "module_user_users", "user", "CONCAT(module_user_users.name_first, ' ', module_user_users.name_last, ' (', module_user_users.user_code, ')') AS user_name", " ORDER BY user_name", false);
}
...
if ($show == "add" || ($do == "add" && is_array($res))) {
    $tpl->addDataItem("TITLE", $txtf->display("module_title_ehis_check"));
    $active_tab = 1;
}
else {
    $tpl->addDataItem("TITLE", $txtf->display("module_title_ehis_check"));
