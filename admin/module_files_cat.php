...
$txtf = new Text($language2, "module_files_cat");

// ##############################################################
// ##############################################################

$table = "files_cat"; // SQL table name to be administered
...
$general = array(
	...
	"max_entries" => 50,
	"sort" => "title ASC" // default sort to use
	//"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
    "group_id" => $txtf->display("group"),
...
$required = array(
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
 	...
 	$what = array(
		"$table.*",
		"files_cat_group.title as grouptitle"
	);
	$from = array(
		$table,
		"LEFT JOIN files_cat_group ON $table.group_id = files_cat_group.id"
	);

	$where = ""; //"language = '$language'";
	
	$filter_fields = array(
		"files_cat_group.title",
		"$table.title"
	);
...
function external() {
	...
	$adm->assignExternal("group_id", "files_cat_group", "id", "title", "ORDER BY title ASC", false);
} 
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
	...
	if ($do == "add") {
		...
		// permissions		
		$perm->Access (0, $id, "d", "realestate");		
	
		$res = $adm->delete($table, $idfield, $id);
		if ($res == 0) { 
			$sq->query($adm->dbc, "DELETE FROM module_forum_text WHERE main = '" . addslashes($id) . "'");
