...
$table = "module_iforum_posts"; // SQL table name to be administered (many possible datasources can be used, but one should be primary)
...
$general = array(
	...
	"max_entries" => 50,
	"sort" => "$table.stamp DESC" // default sort to use
	//"enctype" => "enctype=\"multipart/form-data\""
);

// fields to show in the form
$fields = array(
	"subject"  => $txtf->display("subject")
	,"stamp" => $txtf->display("post_time")
	//,'thread_id' => $txtf->display("thread_name")
	,'content' => $txtf->display("message")
...
$disp_fields = array(
	...
	"subject"  => $txtf->display("subject")
	,"stamp" => $txtf->display("post_time")
	,'thread_name' => $txtf->display("thread_name")
...
// To construct the main list query SELECT what from where / also which fields to include in the Filter command
	...
	$from = array(
		...
		inner join module_iforum_threads t on ($table.thread_id = t.id)
...
function external(&$adm, $show, $id, $language) {
	...
	$adm->assignProp("content", "cols", 60);
	$adm->assignProp("content", "rows", 20);
...
if (empty($show))	$adm->assignProp("forum_id", "java", 'onChange="javascript:this.form.submit();"');
	...
	if ($_SESSION['module_iforum']['admin_forum_id']) {
		$adm->assignFilter("forum_id", $_SESSION['module_iforum']['admin_forum_id'], "f.id = '".addslashes($_SESSION['module_iforum']['admin_forum_id'])."'", $form_select);
	}
	else {
		$adm->assignFilter("forum_id", "", "", $form_select);
	}
...
// DB writes
 ...
 if ($do == "update" && $id) {
		...
		$sq->query($adm->dbc,"select thread_id from module_iforum_posts where id=".intval($id));
		$tmp_tid=$sq->column(0,'thread_id');
