...
$table = "IM_groups"; // SQL table name to be administered

$idfield = "group_id"; // name of the id field (unique field, usually simply 'id')
...
$general = array(
	...
	"max_entries" => 50,
	"sort" => "$table.name ASC" // default sort to use
	//"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
	"name"  => $txtf->display("group_name"),
	 "active" => $txtf->display("group_active")
);
...
if ($show == "add" || ($do == "add" && is_array($res))) {
	$tpl->addDataItem("TITLE", $txtf->display("module_title3"));
	$active_tab = 1;
}
else {
	$tpl->addDataItem("TITLE", $txtf->display("module_title3"));
