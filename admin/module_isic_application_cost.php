...
$table = "module_isic_application_cost"; // SQL table name to be administered
...
$fields = array(
    ...
    "application_type" => $txtf->display("application_type"),
...
function external() {
    global $adm, $show, $txtf, $txt, $group, $language, $id, $structure, $card_type, $application_type;
    //$sq = new sql;

    $adm->assignProp("card_type", "type", "select");
    $adm->assignExternal("card_type", "module_isic_card_type", "id", "name", " WHERE 1=1 ORDER BY name", true);
    assignFilterForField('card_type', $card_type);
    $adm->assignExternal("card_type", "module_isic_card_type", "id", "name", " WHERE 1=1 ORDER BY name", false);

    $adm->assignProp("application_type", "type", "select");
    $adm->assignExternal("application_type", "module_isic_application_type", "id", "name", " ORDER BY name", true);
    assignFilterForField('application_type', $application_type);
    $adm->assignExternal("application_type", "module_isic_application_type", "id", "name", " ORDER BY name", false);
