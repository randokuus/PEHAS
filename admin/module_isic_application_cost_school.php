...
$txtf = new Text($language2, "module_isic_application_cost");

// ##############################################################
// ##############################################################

$table = "module_isic_application_cost_school"; // SQL table name to be administered
...
function assignFilterForField($fieldName, $fieldValue) {
    global $adm, $table;

    $f = new AdminFields($fieldName, $adm->fields[$fieldName]);
    $adm->assignFilter($fieldName, $fieldValue,
        $fieldValue ? "{$table}.{$fieldName}= '" . addslashes($fieldValue) . "'" : '',
        $f->display($fieldValue)
    );
}
...
function external() {
    global $adm, $show, $txtf, $txt, $group, $language, $id, $structure, $school_id;
    //$sq = new sql;

    $adm->assignProp("cost_id", "type", "select");
    $adm->assignExternal("cost_id", "module_isic_application_cost", "module_isic_application_cost.id",
       "CONCAT(module_isic_card_type.name, ': ', module_isic_application_type.name) AS name",
       " , module_isic_card_type, module_isic_application_type WHERE module_isic_application_cost.card_type = module_isic_card_type.id AND module_isic_application_cost.application_type = module_isic_application_type.id ORDER BY name", false);

    $adm->assignProp("school_id", "type", "select");
    $adm->assignExternal("school_id", "module_isic_school", "id", "name", " WHERE 1 = 1 ORDER BY name", true);
    assignFilterForField('school_id', $school_id);
    $adm->assignExternal("school_id", "module_isic_school", "id", "name", " WHERE 1 = 1 ORDER BY name", false);
