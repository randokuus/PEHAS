...
$table = "module_isic_card_data_sync_ccdb"; // SQL table name to be administered
...
*/
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
    ...
    $from = array(
        $table,
        "LEFT JOIN module_isic_card ON ($table.record_id = module_isic_card.id)",
    );
...
function external() {
    ...
    $adm->assignProp("sync_type_id", "list", array("1" => $txtf->display("sync_type1"), "2" => $txtf->display("sync_type2")));

    $adm->assignProp("request", "rows", "10");
...
if ($show == "add" || ($do == "add" && is_array($res))) {
    $tpl->addDataItem("TITLE", $txtf->display("module_title_ccdb"));
    $active_tab = 1;
}
else {
    $tpl->addDataItem("TITLE", $txtf->display("module_title_ccdb"));
