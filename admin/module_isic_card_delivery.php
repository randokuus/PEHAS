...
$txtf = new Text($language2, "module_isic_card_delivery");

// ##############################################################
// ##############################################################

$table = "module_isic_card_delivery"; // SQL table name to be administered
...
$fields = array(
    ...
    "send_notification" => $txtf->display("send_notification"),
    "shipment_id" => $txtf->display("shipment"),
    "send_cost_id" => $txtf->display("send_cost"),
...
function external() {
    ...
    $adm->assignProp("shipment_id", "type", "select");
    $adm->assignExternal("shipment_id", "module_isic_card_shipment", "id", "name", " ORDER BY name", true);

    $adm->assignProp("send_cost_id", "type", "select");
    $adm->assignExternal("send_cost_id", "module_isic_card_send_cost", "module_isic_card_send_cost.id",
        " CONCAT(module_isic_card_send_type.name, ': ', module_isic_card_send_cost.sum, ' ', module_isic_card_send_cost.currency) AS title",
        ", module_isic_card_send_type WHERE module_isic_card_send_cost.send_type_id = module_isic_card_send_type.id ORDER BY title",
        true
    );
    ...
    $adm->assignProp("send_notification", "type", "checkbox");
    $adm->assignProp("send_notification", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));
}
