...
$fields = array(
    "card_producer"  => $txtf->display("card_producer"),
...
function external() {
	...
	$adm->assignExternal("card_type", "module_isic_card_type", "module_isic_card_type.id", "CONCAT(module_isic_card_type.name, ' (', COUNT(*), ')') AS t_name", " , module_isic_card WHERE module_isic_card.type_id = module_isic_card_type.id AND module_isic_card.exported = '0000-00-00' AND module_isic_card.pic <> '' AND module_isic_card.confirm_admin = 1 GROUP BY module_isic_card_type.id ORDER BY module_isic_card_type.name", true);

	$adm->assignProp("card_producer", "type", "select");
	$adm->assignExternal("card_producer", "`module_isic_card_producers`", "`module_isic_card_producers`.`id`", "`module_isic_card_producers`.`name`", "ORDER BY `name`");
	
	$adm->assignProp("school_id", "type", "select");
	$adm->assignExternal("school_id", "module_isic_school", "module_isic_school.id", "CONCAT(module_isic_school.name, ' (', COUNT(*), ')') AS t_name", " , module_isic_card WHERE module_isic_card.school_id = module_isic_school.id AND module_isic_card.exported = '0000-00-00' AND module_isic_card.pic <> '' AND module_isic_card.confirm_admin = 1 GROUP BY module_isic_school.id ORDER BY module_isic_school.name", true);
...
if ($do == "update" && $id && !$lastExportTooClose) {
    ...
    $export_log .= "<input type=\"hidden\" name=\"card_producer\" value=\"$card_producer\">\n";
    $export_log .= "<tr>\n";
    ...
    $producers = IsicDB::factory('CardProducers');
    $producerData = $producers->getRecord($_POST['card_producer']);
    IsicDB::assert($producerData, "Producer not found");
    $card_list = $producers->exportCardsArray($card_type, $school_id, $card_ids);
    if (sizeof($card_list) > 0) {
        ...
        if ($export) {
            $exportedCards = $producers->exportRecordCards($producerData, $card_ids);
        }
