...
$table = "module_isic_card_field"; // SQL table name to be administered
...
$general = array(
    ...
    "max_entries" => 50,
    "sort" => "field" // default sort to use
    //"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
    "field" => $txtf->display("field"),
    "translation" => $txtf->display("translation"),
    "type" => $txtf->display("type"),
    "relation_table" => $txtf->display("relation_table"),
    "relation_field" => $txtf->display("relation_field"),
);
...
function external() {
    ...
    $list = array();
    for ($i = 1; $i <= 5 ; $i++) {
        $list[$i] = $txtf->display("type" . $i);
    }
    $adm->assignProp("type", "type", "select");
    $adm->assignProp("type", "list", $list, false);
}
