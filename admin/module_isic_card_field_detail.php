...
$txtf = new Text($language2, "module_isic_card_field");

// ##############################################################
// ##############################################################

$table = "module_isic_card_field_detail"; // SQL table name to be administered
...
$general = array(
    ...
    "max_entries" => 50,
    "sort" => "sort_order" // default sort to use
    //"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
    "field_id" => $txtf->display("field"),
    "fieldset_id" => $txtf->display("fieldset"),
    "translation" => $txtf->display("translation"),
    "sort_order" => $txtf->display("sort_order"),
    "required" => $txtf->display("required"),
    "disabled" => $txtf->display("disabled"),
    "tooltip" => $txtf->display("tooltip"),
);
...
function external() {
    ...
    /*
    $f_checkbox = array("required", "tooltip");

    foreach ($f_checkbox as $ckey) {
        $adm->assignProp($ckey, "type", "checkbox");
        $adm->assignProp($ckey, "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));
    }
    */

    $adm->assignProp("field_id", "type", "select");
    $adm->assignExternal("field_id", "module_isic_card_field", "id", "field", " ORDER BY field", true);

    $adm->assignProp("fieldset_id", "type", "select");
    $adm->assignExternal("fieldset_id", "module_isic_card_fieldset", "id", "name", " ORDER BY name", true);

    for ($i = 1; $i <= 4 ; $i++) {
        $list[$i] = $txtf->display("action" . $i);
    }
    $adm->assignProp("required", "type", "select2");
    $adm->assignProp("required", "size", 5);
    $adm->assignProp("required", "list", $list, false);

    $adm->assignProp("disabled", "type", "select2");
    $adm->assignProp("disabled", "size", 5);
    $adm->assignProp("disabled", "list", $list, false);
