...
$table = "module_isic_card_log"; // SQL table name to be administered
...
$required = array(
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
    ...
    $what = array(
        "$table.*",
        "CONCAT(module_isic_card.person_name_first, ' ', module_isic_card.person_name_last, ' (',  module_isic_card.isic_number, ')') as card_name"
    );
    $from = array(
        $table,
...
function external() {
    ...
    // $adm->assignExternal("card_id", "module_isic_card", "id", "
    //     CONCAT(module_isic_card.person_name_first, ' ', module_isic_card.person_name_last, ' (', module_isic_card.isic_number, ')') AS tmp_card_name ", 
    //     " ORDER BY module_isic_card.person_name_last", false);
/*
