...
$txtf = new Text($language2, "module_isic_card_number");

// ##############################################################
// ##############################################################

$table = "module_isic_card_number"; // SQL table name to be administered
...
$general = array(
    ...
    "max_entries" => 50,
    "sort" => "reserved_date DESC, entrydate DESC" // default sort to use
...
$fields = array(
    ...
    "reserved_date" => $txtf->display("reserved_date"),
);
...
function addNumberCountsForCardType($typeList) {
    global $database, $txtf;
    foreach ($typeList as $typeId => $typeName) {
        if (!$typeId) {
            continue;
        }
        $res = $database->query('SELECT count(*) AS total FROM module_isic_card_number WHERE card_type = ! AND reserved = 0', $typeId);
        $data = $res->fetch_assoc();
        if ($data) {
            $typeList[$typeId] = $typeName . ' (' . $txtf->display('free') . ': ' . $data['total'] . ')';
        }
    }
    return $typeList;
}    
...
function external() {
    global $adm, $show, $txtf, $txt, $group, $language, $id, $structure, $table, $card_type;
    //$sq = new sql;
	...
	$adm->displayOnly("entrydate");

    $adm->assignProp("reserved", "type", "checkbox");
    $adm->assignProp("reserved", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));

	$adm->assignProp("card_type", "type", "select");
	$adm->assignExternal("card_type", "module_isic_card_type", "id", "name", " WHERE number_type = 1 ORDER BY name", true);

    $adm->fields['card_type']['list'] = addNumberCountsForCardType($adm->fields['card_type']['list']);
	
    $fdata = $adm->fields["card_type"];
    $f = new AdminFields("card_type", $fdata);
    $form_select = $f->display($card_type);
    
    
    if ($card_type) {
        $adm->assignFilter("card_type", $card_type, "$table.card_type = '" . addslashes($card_type) . "'", $form_select);
    }
    else {
        $adm->assignFilter("card_type", "", "", $form_select);
    }
