...
$txtf = new Text($language2, "module_isic_card_send_cost");

// ##############################################################
// ##############################################################

$table = "module_isic_card_send_cost"; // SQL table name to be administered
...
$fields = array(
    "send_type_id" => $txtf->display("send_type"),
...
function external() {
    ...
    $adm->assignProp("send_type_id", "type", "select");
    $adm->assignExternal("send_type_id", "module_isic_card_send_type", "id", "name", " ORDER BY name", true);
}
