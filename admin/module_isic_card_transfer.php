...
$txtf = new Text($language2, "module_isic_card_transfer");

// ##############################################################
// ##############################################################

$table = "module_isic_card_transfer"; // SQL table name to be administered
...
$fields = array(
    "order_name" => $txtf->display("order_name"),
    "entrydate" => $txtf->display("entrydate"),
    "chip_file" => $txtf->display("chip_file"),
    "import_time" => $txtf->display("import_time"),
...
function external() {
    ...
    if ($adm->fields['order_name']['value']) {
        $extra = '<a href="download.php?f=/cache/isic/' .
            str_replace('.txt', '.tar', $adm->fields['order_name']['value']) .
            '"><img src="pic/icosmall_other.gif" align="absmiddle" alt="Download" title="Download" border=0></a>';
        $adm->assignProp('order_name', 'extra', $extra);
    }
}
