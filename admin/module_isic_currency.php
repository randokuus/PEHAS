...
$txtf = new Text($language2, "module_isic_currency");

// ##############################################################
// ##############################################################

$table = "module_isic_currency"; // SQL table name to be administered
...
$fields = array(
    ...
    "is_default" => $txtf->display("is_default"),
    "rate" => $txtf->display("rate"),
);
...
function external() {
    ...
    $adm->assignProp("is_default", "type", "checkbox");
    $adm->assignProp("is_default", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));
}

function setCurrenciesNotDefault($id) {
    global $adm, $table;
    $sq = new sql;
    $sql = "UPDATE " . $table . " SET is_default = 0 WHERE id <> '" . $id . "'";
    $sq->query($adm->dbc, $sql);
}

function setRatesForNotDefaultCurrencies($id) {
    global $adm, $table;
    $sq = new sql;
    ...
    $sql = "UPDATE {$table} SET rate = rate / " . $rate . " WHERE id <> " . $id;
    $sq->query($adm->dbc, $sql);
    
    $sql = "UPDATE {$table} SET rate = 1 WHERE id = " . $id;
    $sq->query($adm->dbc, $sql);
}

function isValidRate($sum) {
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
    ...
    if ($do == "add") {
        ...
        $adm->values['is_default'] = 0;
        if (isValidRate($_POST['rate'])) {
            $res = $adm->add($table, $required, $idfield);
        } else {
            $adm->checkRequired($required);
            $adm->badfields[] = 'rate';
            $res = $adm->badfields;
        }
        ...
        if ($res == 0) {
            ...
            if ($adm->fields['is_default']['value']) {
                setCurrenciesNotDefault($id);
                setRatesForNotDefaultCurrencies($id);
            }
        }
        else {
            $tpl->addDataItem("NOTICE", "onLoad=\"notice('" . $general["required_error"] . "')\"");
