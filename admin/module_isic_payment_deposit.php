...
$txtf = new Text($language2, "module_isic_payment_deposit");

// ##############################################################
// ##############################################################

$table = "module_isic_payment_deposit"; // SQL table name to be administered
...
$fields = array(
    "card_appl" => $txtf->display("card_appl"),
    "card_id" => $txtf->display("card_id"),
    "event_time" => $txtf->display("event_time"),
    "event_user" => $txtf->display("event_user"),
    "transaction_number" => $txtf->display("transaction_number"),
    "bank_id" => $txtf->display("bank"),
    "amount" => $txtf->display("amount"),
    "currency" => $txtf->display("currency"),
    "rec_account" => $txtf->display("rec_account"),
    "rec_name" => $txtf->display("rec_name"),
    "snd_account" => $txtf->display("snd_account"),
    "snd_name" => $txtf->display("snd_name"),
    "ref_number" => $txtf->display("ref_number"),
    "message" => $txtf->display("message"),
    "transaction_date" => $txtf->display("transaction_date"),
);
...
//    "ref_number",
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
    ...
    $where = "{$table}.event_user = module_user_users.user";

    $filter_fields = array(
        "$table.transaction_number",
        "$table.rec_name",
        "$table.snd_name",
        'module_user_users.name_first',
        'module_user_users.name_last',
...
function external() {
    ...
    $adm->assignExternal("event_user", "module_user_users", "user", "CONCAT(name_last, ' ', name_first, ' (', username, ')') as name", " ORDER BY name", false);
    
    $cardApplList = array("1" => $txtf->display("card_appl1"), "2" => $txtf->display("card_appl2"));
    $adm->assignProp("card_appl", "type", "select");
    $adm->assignProp("card_appl", "list", $cardApplList);
    
    $bankList = array("1" => $txtf->display("bank1"), "2" => $txtf->display("bank2"), "3" => $txtf->display("bank3"), "4" => $txtf->display("bank4"));
    $adm->assignProp("bank_id", "type", "select");
    $adm->assignProp("bank_id", "list", $bankList);
