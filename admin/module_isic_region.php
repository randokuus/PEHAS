...
$txtf = new Text($language2, "module_isic_region");

// ##############################################################
// ##############################################################

$table = "module_isic_region"; // SQL table name to be administered
...
$general = array(
    ...
    "error_nonempty" => $txt->display("error_nonempty"),
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
    ...
    if ($do == "add") {
        ...
        /** @var IsicDB_Schools $dbSchool */
        $dbSchool = IsicDB::factory('Schools');
        $regionSchools = $dbSchool->findRecords(array('region_id' => $id));
        if (count($regionSchools) == 0) {
