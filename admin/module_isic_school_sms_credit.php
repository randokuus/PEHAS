...
$txtf = new Text($language2, "module_isic_school_sms_credit");

// ##############################################################
// ##############################################################

$table = "module_isic_school_sms_credit"; // SQL table name to be administered
...
$fields = array(
    ...
    "credit" => $txtf->display("credit"),
    "reserved" => $txtf->display("reserved"),
);
...
function external() {
    ...
    $adm->assignProp('credit', 'extra', $txtf->display('credit_extra'));
}
...
/** @var IsicDB_SchoolSMSCredit $isicSchoolSMSCredit */
$isicSchoolSMSCredit = IsicDB::factory('SchoolSMSCredit');
/** @var IsicDB_GlobalSettings $globalSettings */
$globalSettings = IsicDB::factory('GlobalSettings');
$globalSmsCredit = floatval($globalSettings->getRecord('sms_credit'));
...
if ($do) {
    $recordId = $id;
    $schoolId = $adm->values['school_id'];
    $newCredit = floatval($adm->values['credit']);
    $creditData = $isicSchoolSMSCredit->getRecord($recordId);
    if ($creditData) {
        $oldCredit = floatval($creditData['credit']);
    } else {
        $oldCredit = 0;
    }
    $schoolCreditData = $isicSchoolSMSCredit->getRecordBySchoolId($schoolId);
}
    ...
    if ($do == "add") {
        ...
        if ($newCredit > $globalSmsCredit) {
            $adm->badfields = array();
            $adm->general['error'] = 'Credit too large';
            $adm->general['other_error'] = 'Current credit (' . $newCredit . ') exceeds global limit (' . $globalSmsCredit . ')';
            $res = 1;
        } else if ($schoolCreditData) {
            $adm->badfields = array();
            $adm->general['error'] = 'School exists';
            $adm->general['other_error'] = 'Credit record for this school already exists';
            $res = 1;
        } else {
        ...
        if ($res == 0) {
            ...
            $globalSettings->setValue('sms_credit', $globalSmsCredit - $newCredit);
        } else {
        ...
        if ($schoolCreditData && $schoolCreditData['id'] != $creditData['id']) {
            $adm->badfields = array();
            $adm->general['error'] = 'School exists';
            $adm->general['other_error'] = 'Credit record for this school already exists (duplicates not allowed)';
            $res = 1;
        } else if ($globalSmsCredit < ($newCredit - $oldCredit)) {
            $adm->badfields = array();
            $adm->general['error'] = 'Credit too large';
            $adm->general['other_error'] = 'Current credit (' . $newCredit . ') exceeds global limit (' . $globalSmsCredit . ')';
            $res = 1;
        ...
        if ($res == 0) {
            ...
            $globalSettings->setValue('sms_credit', $globalSmsCredit - ($newCredit - $oldCredit));
        } else {
            $tpl->addDataItem("NOTICE", "onLoad=\"notice('" . $general["required_error"] . "')\"");
            $adm->getValues();
            $adm->types();
        ...
        if ($res == 0) {
            ...
            $globalSettings->setValue('sms_credit', $globalSmsCredit + $oldCredit);
        } else {
