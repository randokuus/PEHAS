...
$txtf = new Text($language2, "module_isic_service_event");

// ##############################################################
// ##############################################################

$table = "module_isic_service_event"; // SQL table name to be administered
...
$fields = array(
    "location_id" => $txtf->display("location"),
    "event_id" => $txtf->display("event"),
    "device_id" => $txtf->display("device"),
    "card_id" => $txtf->display("card"),
    "event_time" => $txtf->display("event_time"),
//    "direction" => $txtf->display("direction"),
//    "access" => $txtf->display("access"),
    "sale_sum" => $txtf->display("sale_sum"),
    "discount_sum" => $txtf->display("discount_sum"),
...
//    $adm->assignProp("access", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));
	...
	$adm->assignProp("location_id", "type", "select");
	$adm->assignExternal("location_id", "module_isic_service_location", "id", "name", " ORDER BY name", false);

    $adm->assignProp("device_id", "type", "select");
    $adm->assignExternal("device_id", "module_isic_service_device", "id", "name", " ORDER BY name", false);
	
	$adm->assignProp("card_id", "type", "select");
	$adm->assignExternal("card_id", "module_isic_card", "id", "isic_number", " ORDER BY isic_number", false);
}
