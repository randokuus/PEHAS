...
$txtf = new Text($language2, "module_isic_service_location");

// ##############################################################
// ##############################################################

$table = "module_isic_service_location"; // SQL table name to be administered
...
$fields = array(
    "provider_id" => $txtf->display("provider"),
    ...
    "card_type_list" => $txtf->display("card_type_list"),
);
...
function external() {
    ...
    $adm->assignProp("provider_id", "type", "select");
    $adm->assignExternal("provider_id", "module_isic_service_provider", "id", "name", " ORDER BY name", false);

	$adm->assignProp("school_list", "type", "select2");
	$adm->assignProp("school_list", "size", "20");
	$adm->assignExternal("school_list", "module_isic_school", "id", "name", " ORDER BY name", true);

    $adm->assignProp("card_type_list", "type", "select2");
    $adm->assignProp("card_type_list", "size", "20");
    $adm->assignExternal("card_type_list", "module_isic_card_type", "id", "name", " ORDER BY name", false);
}
