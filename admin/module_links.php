...
$txtf = new Text($language2, "module_links");

// ##############################################################
// ##############################################################

$table = "module_links"; // SQL table name to be administered
...
$general = array(
	...
	"required_error" => $txtf->display("unique_error"),
...
$fields = array(
	"lgroup" => $txtf->display("lgroup"),
	"linkid"  => $txtf->display("linkid"),
	"redirectto" => $txtf->display("redirectto"),
	"description" => $txtf->display("description"),
	"active" => $txtf->display("active"),
	"school_list" => $txtf->display("school_list")
);
...
/*
	if ($show == "modify") {
		$adm->displayOnly("linkid");
	}
*/
	$adm->assignProp("redirectto", "extra", "&nbsp;<a href=\"javascript:newWindow('select_hyperlink.php', 500,320);\"><img align=top src=\"pic/link.gif\" WIDTH=\"23\" HEIGHT=\"22\" border=0 alt=\"\"></a>");

	$adm->assignProp("active", "type", "checkbox");
	$adm->assignProp("active", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));

	$adm->assignProp("lgroup", "type", "select");
	$adm->assignExternal("lgroup", "module_links_groups", "id", "name", " WHERE language = '$language' ORDER BY name ASC", false);

	$adm->assignProp("school_list", "type", "select2");
	$adm->assignProp("school_list", "size", 10);
	$adm->assignExternal("school_list", "module_isic_school", "id", "name", " ORDER BY name", true);
}
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
	...
	if ($do == "add") {
		...
		// permissions
		$perm->Access (0, 0, "a", "links");
/*
		$sq->query($adm->dbc, "SELECT linkid FROM module_links WHERE language = '" . addslashes($adm->values["linkid"]) . "'");
		if ($sq->numrows != 0) $adm->values["linkid"] = "";

		if ($adm->values["linkid"]) {
			$linkid_new = preg_replace('/ /', '_', $adm->values["linkid"]);
			$linkid_new = iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $linkid_new);
			$linkid_new = preg_replace('/\$|%|@|&|�|�|�|�|�|�|�|�|\"|\'/', '', $linkid_new);
			if ($adm->values["linkid"] != $linkid_new) {
				$adm->values["linkid"] = "";
			}
		}
*/
