...
$txtf = new Text($language2, "module_messages_send_log");

// ##############################################################
// ##############################################################

$table = "module_messages_send_log"; // SQL table name to be administered
...
$general = array(
    ...
    "max_entries" => 50,
    "sort" => "sendtime DESC" // default sort to use
    //"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
    "sendtime" => $txtf->display("sendtime"),
    "user_id" => $txtf->display("user"),
    "school_id" => $txtf->display("school"),
    "from" => $txtf->display("from"),
    "to" => $txtf->display("to"),
    "text" => $txtf->display("text"),
    "request" => $txtf->display("request"),
    "response" => $txtf->display("response"),
    "message-id" => $txtf->display("message-id"),
    "status" => $txtf->display("status"),
    "remaining_balance" => $txtf->display("remaining_balance"),
    "message_price" => $txtf->display("message_price"),
    "network" => $txtf->display("network"),
    "error_text" => $txtf->display("error_text"),
);
...
$from = array(
    'module_user_users, ',
    $table,
    " LEFT JOIN module_isic_school ON {$table}.school_id = module_isic_school.id"
);

$where = "{$table}.user_id = module_user_users.user";
...
function external() {
    ...
    $adm->assignProp("text", "rows", "2");
    $adm->assignProp("text", "cols", "100");

    $adm->assignProp("request", "rows", "3");
    $adm->assignProp("request", "cols", "100");

    $adm->assignProp("response", "rows", "15");
    $adm->assignProp("response", "cols", "100");

    $adm->assignProp("error_text", "rows", "2");
    $adm->assignProp("error_text", "cols", "100");

    $adm->assignProp("user_id", "type", "select");
    $adm->assignExternal("user_id", "module_user_users", "user", "CONCAT(name_last, ' ', name_first, ' (', username, ')') AS name", "", false);
    $adm->displayOnly("user_id");
