...
$txtf = new Text($language2, "module_news_config");
...
$table = "module_news_config"; // SQL table name to be administered
...
$fields = array(
    ...
    "language" => $txtf->display("language"),
    "url"  => $txtf->display("url"),
    "username"  => $txtf->display("username"),
    "password"  => $txtf->display("password"),
    "path" => $txtf->display("path"),
    "import_from" => $txtf->display("import_from"),
...
function external() {
    ...
    $adm->assignExternal("ngroup", "module_news_groups", "id", "CONCAT(name, ' (', language, ')') AS name", "ORDER BY name ASC", true);

    $adm->assignProp("active", "type", "checkbox");
    $adm->assignProp("active", "list", array("0" => $txtf->display("active0"), "1" => $txtf->display("active1")));
