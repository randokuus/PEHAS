...
$table = "module_poll_questions"; // SQL table name to be administered
...
$general = array(
	...
	"template_form" => "tmpl/admin_form1.html",  //form 2
	...
	"max_entries" => 100,
	"sort" => "prio DESC" // default sort to use
	//"enctype" => "enctype=\"multipart/form-data\""
);

/* the fields in the table */
$fields = array(
	"poll" => $txtf->display("poll"),
	"question" => $txtf->display("question"),
	"score" => $txtf->display("score"),
	"prio" => $txtf->display("prio")
);

$tabs = array(
	1 => array($txt->display("add"), $_SERVER["PHP_SELF"]."?show=add&poll=$poll"),
	2 => array($txt->display("modify"), $_SERVER["PHP_SELF"]."?poll=$poll")
);
...
$required = array(
 ...
 /* To construct the main list query SELECT what from where / also which fields to include in the Filter command*/
 	...
 	$what = array(
		"$table.*",
		"module_poll.title as pollname"
	);
	$from = array(
		$table,
		"LEFT JOIN module_poll ON $table.poll = module_poll.id"
	);

	$where = ""; //"poll = '" . addslashes($poll) . "'";
...
function external() {
	global $adm, $show, $txtf, $txt, $group, $table, $language, $id, $structure, $poll;
	$sq = new sql;
	...
	$adm->assignProp("prio", "extra", $txtf->display("prio1"));

	$adm->assignProp("poll", "type", "select");
	$adm->assignExternal("poll", "module_poll", "id", "title", "WHERE language = '$language' ORDER BY start_time DESC", false);

	$fdata = $adm->fields["poll"];
//	$fdata["java"] = "onChange=\"this.form.submit()\"";
	$f = new AdminFields("poll",$fdata);
	$poll_select = $f->display($poll);

	if ($poll) {
		$adm->assignFilter("poll", $poll, "$table.poll = '".addslashes($poll)."'", $poll_select);
	}
	else {
		$test = $adm->fields["poll"]["list"];
		if (is_array($test)) {
			list($key, $val) = each($test);
			if ($key) {
				$adm->assignFilter("poll", $key, "$table.poll = '$key'", $poll_select);
			}
		}
		else {
			$adm->assignFilter("poll", "", "", $poll_select);
		}
	}
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
	...
	//$adm->assign("poll", $poll);
	//$adm->assignHidden("poll", $poll);
	//$tpl->addDataItem("HIDDEN", "poll=$poll");
...
if ($poll) {
	$tpl->addDataItem("HIDDEN", "$table.poll=$poll");
}
else {
	$tpl->addDataItem("HIDDEN", "$table.poll=".$adm->extra_filter["poll"][0]);
}


if ($show == "add" || ($do == "add" && is_array($res))) {
	$tpl->addDataItem("TITLE", $txtf->display("title1"));
	$active_tab = 1;
}
else {
	$tpl->addDataItem("TITLE", $txtf->display("title1"));
