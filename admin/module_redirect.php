...
$txtf = new Text($language2, "module_redirect");

// ##############################################################
// ##############################################################

$table = "module_redirect"; // SQL table name to be administered
...
$tpl->addDataItem("CONFIRMATION", $general["delete_confirmation"]);
	...
	if ($do == "add") {
		...
		// permissions
		$perm->Access (0, 0, "a", "redirect");

		$sq->query($adm->dbc, "SELECT linkid FROM module_redirect WHERE language = '" . addslashes($adm->values["linkid"]) . "'");
