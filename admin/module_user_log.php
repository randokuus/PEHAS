...
$txtf = new Text($language2, "module_user_log");

// ##############################################################
// ##############################################################

$table = "module_user_users_log"; // SQL table name to be administered
...
function external() {
    ...
    $adm->assignExternal("user_id", "module_user_users", "user", "CONCAT(module_user_users.name_first, ' ', module_user_users.name_last, ' (', module_user_users.user_code, ')') AS user_name", " ORDER BY user_name", false);
}
