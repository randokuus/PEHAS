...
$txtf = new Text($language2, "module_user_status");

// ##############################################################
// ##############################################################

$table = "module_user_status"; // SQL table name to be administered
...
$fields = array(
    ...
    "card_types" => $txtf->display("card_types"),
    "ehis_check" => $txtf->display("ehis_check"),
    "ehl_check" => $txtf->display("ehl_check"),
);
...
function external() {
	...
	$adm->assignProp("card_types", "size", 10);
	$adm->assignExternal("card_types", "module_isic_card_type", "id", "name", " ORDER BY name", false);
	
    $adm->assignProp("ehis_check", "type", "checkbox");
    $adm->assignProp("ehis_check", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));

    $adm->assignProp("ehl_check", "type", "checkbox");
    $adm->assignProp("ehl_check", "list", array("0" => $txt->display("no"), "1" => $txt->display("yes")));
}
