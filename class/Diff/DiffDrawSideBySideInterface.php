...
/**
 * Diff show in format "SideBySide" interfase
 *
 * @author Victor Buzyka <victor@itworks.biz.ua>
 */
class DiffDrawSideBySideInterface extends DiffDrawInterface
{
    /**
     * Draw added strings
     *
    ...
    function Modifi($old_pos, $new_pos)
    {
        ...
        $class = $this->settings->getStyle('modifi');
        while ($s_pos <= $end_pos || $o_s_pos <= $o_end_pos) {
            ...
            $res .= "<tr> "
                . "<td class=\"" .$this->settings->getStyle('position'). "\"> " . $o_pos . " </td>"
    ...
    function Delete($old_pos, $new_pos)
    {
        ...
        //$tmp_pos = $new_pos - ($old_pos['end'] - $old_pos['start']);
