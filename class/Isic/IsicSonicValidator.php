...
class IsicSonicValidator extends IsicValidator {
    protected $lineElementsCount = 18;
    ...
    public function isValidLine(array $line) {
        ...
        if (!$this->isValidPersonNumber($line[2])) {
            $this->addError('person_number', $line[2]);
        }
        if (!$this->isValidBirthday($line[3])) {
            $this->addError('birthday', $line[3]);
        }
