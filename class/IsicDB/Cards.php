...
class IsicDB_Cards extends IsicDB {
    /**
     * States for cards
     */
    ...
    */
    ...
    public function saveChipAndPanNumber($isicNumber, $chipNumber, $panNumber, $orderId = 0) {
        ...
        if (!$data) {
            return array(
                'status' => false,
                'message' => "Chip/Pan numbers not assigned. Couldn't find the card {$isicNumber}."
            );
        }
        ...
        return array(
            'status' => true,
            'message' => "Chip: {$chipNumber}; Pan: {$panNumber} assigned to {$isicNumber} (id: {$data['id']}) (order: {$orderId})."
        );
    }
    ...
    private function saveFieldValue($fieldName, $fieldValue, $isicNumber, $orderId = 0) {
        ...
        if (!$data) {
            $this->saveStatus = "<b>{$fieldName} number not assigned.</b> Couldn't find the card.";
            return false;
        }

        if ($data[$fieldName . "_number"] === $fieldValue) {
            $this->saveStatus = "{$fieldName} number already assigned.";
            return false;
        }

        $this->updateRecord($data['id'], array($fieldName . '_number' => $fieldValue));
        $this->saveStatus = "{$fieldName} number assigned.";
        return $data['id'];
    }
    ...
    public function getCardsWithoutChipNumber($order_id) {
        ...
        $card_list = array();
        $r = $this->db->query(
            'SELECT `isic_number`, `chip_number` FROM ?f WHERE `order_id` = !',
            $this->table,
            $order_id
        );
        while ($data = $r->fetch_assoc()) {
            if (!$data["chip_number"]) {
                $card_list[] = $data["isic_number"];
            }
        }
        return $card_list;
    }
