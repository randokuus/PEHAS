...
* Last Modified..: $Date: 2010-07-01 15:47:10 +0300 (Thu, 01 Jul 2010) $
* CVS Revision...: $Revision: 2636 $
* @package modera_net
*/

require_once(SITE_PATH . '/class/mail/mimePart.php');
require_once(SITE_PATH . '/class/mail/MailLog.php');
	...
	function send($recipients, $type = 'mail')
	{
		...
		switch ($type) {
			case 'mail':
				...
				{
				    $headers[] = 'Reply-To : ' . $this->return_path;
				}
				...
				$this->mailLog->clearLogData();
                $this->mailLog->setValue('To', $to);
                $this->mailLog->setValue('Subject', $subject);
                $this->mailLog->setValue('Headers', implode("\n", $headers));
                $this->mailLog->setValue('Message', $this->output);
                $this->mailLog->setValue('Result', $result);
                $this->mailLog->save();
			...
			case 'smtp':
				require_once(SITE_PATH . '/class/mail/smtp.php');
				require_once(SITE_PATH . '/class/mail/RFC822.php');
