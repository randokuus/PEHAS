...
class isic_application_pic {
     ...
     * @access protected
     */
    var $translation_module_default = "module_isic_application";

    var $isicDbApplications = false;
   ...
   /**
     ...
     *
     * @return string html isic_application_pic content
    */
    ...
    function show() {
        ...
        if (!$this->userid) {
            trigger_error("Module 'isic_application_pics' requires an authorized user. Configure the site to be password protected.", E_USER_ERROR);
        }
    ...
    */
    ...
    function addFile() {
        ...
        if ($this->multiUploadSupport()) {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_application_pic_madd.html";
        } else {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_application_pic_sadd.html";
        }

        $tpl->setInstance($_SERVER["PHP_SELF"]."?language=".$this->language."&module=isic_application_pic&faction=add");
        $tpl->setTemplateFile($template);

        // PAGE CACHED
        if ($tpl->isCached($template) == true && $usecache == true) {
            $GLOBALS["caching"][] = "isic_application_pic";
            return "<!-- module isic_application_pic cached -->\n" . $tpl->parse();
        }
    ...
    */
    ...
    function log_faction($data) {
        if ($fp_log = fopen(SITE_PATH . "/isic_application_pic.txt", "a+")) {
...
//            unset($_SESSION["upload"][$form_id]);
        ...
        if ($write) {
            ...
            if (is_array($pic_list)) {
                ...
                foreach ($pic_list as $pic_name => $val) {
                    ...
                    $res =& $this->db->query("
                        SELECT * FROM `module_isic_application`
                        WHERE
                            `module_isic_application`.`school_id` IN (!@) AND
                            `module_isic_application`.`person_number` = ? AND
                            `module_isic_application`.`state_id` <> !
                        ORDER BY `module_isic_application`.`moddate` DESC LIMIT 1",
                        $this->allowed_schools,
                        $person_number,
                        $this->isic_common->a_state_processed
                    );

                    $appl_data = $res->fetch_assoc();
                    if ($appl_data) {
                        ...
                        if (is_file($src_file) && is_readable($src_file)) {
                            $pics_saved++;
                            $tar_fname = $this->pic_prefix . str_pad($appl_data["id"], 10, '0', STR_PAD_LEFT) . ".jpg";
                            $tar_fname_thumb = $this->pic_prefix . str_pad($appl_data["id"], 10, '0', STR_PAD_LEFT) . "_thumb.jpg";
                            ...
                            $this->isicDbApplications->updateRecord($appl_data["id"], array('pic' => $picName));
                            $this->isic_common->copyApplicationPictureToUser($this->isicDbApplications->getRecord($appl_data['id']));
                        }
                    }
                }
                $success = true;
        ...
        if ($write) {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_application_pic_success.html";
        } else {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_application_pic_list.html";
        }

        $tpl->setInstance($_SERVER["PHP_SELF"]."?language=".$this->language."&module=isic_application_pic&faction=finish_upload");
        $tpl->setTemplateFile($template);
            ...
            if (is_array($files_list)) {
                ...
                for ($i = 0; $i < sizeof($files_list); $i++) {
                    ...
                    if ($person_number) {
                        $res =& $this->db->query("
                            ...
                            WHERE
                                ...
                                `module_isic_application`.`person_number` = ? AND
                                `module_isic_application`.`state_id` <> !
                            ORDER BY `module_isic_application`.`moddate` LIMIT 1",
                            $this->allowed_schools,
                            $person_number,
                            $this->isic_common->a_state_processed
                        );
                    ...
                    if (is_array($appl_data)) {
                        ...
                        $tpl->addDataItem("ROW.PERSON_PIC_OLD", $appl_data["pic"] ? "<img src=\"" . SITE_URL . $appl_data["pic"] . "\">" : "-");
                        $tpl->addDataItem("ROW.PERSON_NAME", $appl_data["person_name_first"] . ' ' . $appl_data["person_name_last"]);
                        $tpl->addDataItem("ROW.PERSON_NUMBER", $appl_data["person_number"]);
