...
class isic_pic {
   ...
   /**
     ...
     *
     * @return string html isic_pic content
    */
    ...
    function show() {
        ...
        if (!$this->userid) {
            trigger_error("Module 'isic_pics' requires an authorized user. Configure the site to be password protected.", E_USER_ERROR);
        }
    ...
    */
    ...
    function addFile() {
        ...
        $txt = new Text($this->language, "module_isic_pic");
        ...
        if ($this->multiUploadSupport()) {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_pic_madd.html";
        } else {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_pic_sadd.html";
        }

        $tpl->setInstance($_SERVER["PHP_SELF"]."?language=".$this->language."&module=isic_pic&faction=add");
        $tpl->setTemplateFile($template);

        // PAGE CACHED
        if ($tpl->isCached($template) == true && $usecache == true) {
            $GLOBALS["caching"][] = "isic_pic";
            return "<!-- module isic_pic cached -->\n" . $tpl->parse();
        }
...
//            unset($_SESSION["upload"][$form_id]);
        ...
        if ($write) {
            ...
            if (is_array($pic_list)) {
                ...
                foreach ($pic_list as $pic_name => $val) {
                    $person_number = $pic_name;

                    $res =& $this->db->query("
                        SELECT * FROM `module_isic_card`
                        WHERE
                            `module_isic_card`.`school_id` IN (!@) AND
                            `person_number` = ? AND
                            `exported` = ?
                        ORDER BY `moddate` DESC LIMIT 1", $this->allowed_schools, $person_number, '0000-00-00');

                    if ($person_data = $res->fetch_assoc()) {
                        ...
                        if (is_file($src_file) && is_readable($src_file)) {
                            $pics_saved++;
                            if ($person_data["pic"]) {
                                $cp_status = @copy($src_file, SITE_PATH . $person_data["pic"]);
                                $cp_status = @copy($src_file_thumb, SITE_PATH . str_replace(".jpg", "_thumb.jpg", $person_data["pic"]));
                            } else {
                                $tar_fname = 'ISIC' . str_pad($person_data["id"], 10, '0', STR_PAD_LEFT) . ".jpg";
                                $tar_fname_thumb = 'ISIC' . str_pad($person_data["id"], 10, '0', STR_PAD_LEFT) . "_thumb.jpg";
                                $cp_status = @copy($src_file, $tar_path . $tar_fname);
                                $cp_status = @copy($src_file_thumb, $tar_path . $tar_fname_thumb);
                                $res =& $this->db->query("UPDATE `module_isic_card` SET `pic` = ? WHERE `id` = !", "/" . $GLOBALS["directory"]["upload"] . "/" . $this->folder_real . $tar_fname, $person_data["id"]);
                            }
                        }
                    }
                }
        ...
        if ($write) {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_pic_success.html";
        } else {
            $template = $GLOBALS["templates_".$this->language][$this->tmpl][1] . "/" . "module_isic_pic_list.html";
        }

        $tpl->setInstance($_SERVER["PHP_SELF"]."?language=".$this->language."&module=isic_pic&faction=finish_upload");
        $tpl->setTemplateFile($template);
                ...
                for ($i = 0; $i < sizeof($files_list); $i++) {
                    ...
                    if (is_array($person_data)) {
                        ...
                        if (($pic_size[0] + 1) >= $this->pic_size_x && ($pic_size[1] + 1) >= $this->pic_size_y) {
                            $fdata["type"] = "checkbox";
                            $f = new AdminFields("save_pic[" . $person_number . "]", $fdata);
                        ...
                        $tpl->addDataItem("ROW.PERSON_NAME", $person_data["person_name_first"] . ' ' . $person_data["person_name_last"]);
                        $tpl->addDataItem("ROW.PERSON_NUMBER", $person_data["person_number"]);
                        ...
                        $tpl->addDataItem("ROW.PERSON_NUMBER", "-");
            ...
            if ($filetype == "jpg") {
                ...
                if (!$return_val) {
                    ...
                    if (is_array($picsize) && (image_type_to_mime_type($picsize[2]) == "image/jpeg")) {
                        if ($image_size) {
                            $is = explode("x", $image_size);
                            $image_size_x = $is[0];
                            $image_size_y = $is[1];

                            $pic_ratio = $picsize[0] / $picsize[1];

                            if ($pic_ratio > $image_size_x / $image_size_y) {
                                // landscape, picture will be resized according to height
                                $t_image_size = "x" . $image_size_y;
                                $resize_ratio = $image_size_y / $picsize[1];
                                $t_image_shave = round((round($resize_ratio * $picsize[0]) - $image_size_x) / 2) . "x0";
                            } else {
                                // portrait, picgure will be resized according to width
                                $t_image_size = $image_size_x;
                                $resize_ratio = $image_size_x / $picsize[0];
                                $t_image_shave = "0x" . round((round($resize_ratio * $picsize[1]) - $image_size_y) / 2);
                            }
                            $stat = @system(IMAGE_CONVERT . " -geometry \"" . $t_image_size . ">\" $pilta $pilta", $kala2);
                            $stat = @system(IMAGE_CONVERT . " -shave \"" . $t_image_shave . "\" $pilta $pilta", $kala2);
