...
if (defined("PANGALINK_MOCK")) {
    require_once(SITE_PATH . "/class/Pangalink/Danske_Mock.php");
} else {
    require_once(SITE_PATH . "/class/Pangalink/Danske.php");
}
...
//    $tpl->setTemplateFile(SITE_PATH . "/tmpl/module_pangalink_form.html");
    ...
    $url = SITE_URL . "/danske/id/" . $id;
    $pangalink = new Pangalink_Danske();
...
//    $res['VK_INFO'] = $isicEncoding->convertStringEncoding($res['VK_INFO']);

    $error = $vkInfo ? 0 : 5;
    if (!$error && !$usr->setDanskeSession($id, $vkInfo)) {
        $error = $usr->errorCode;
    }
...
// Danske authorization should be enabled
header('Location: ' . SITE_URL . '/?auth_type=5&auth_error=1&error_code=' . $error);
