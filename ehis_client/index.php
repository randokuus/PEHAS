...
if($isSearch)
{
    $searchServer = @$_POST['server'];
    $par = new eyl_isic_paring();
    $par->data = $data;

    if ($searchServer == 'live') {
        $wsdl = 'adapter.wsdl';    
    } else {
        $wsdl = 'adapter_test.wsdl';
    }
...
?><html>
    <head>
            <title>EhisClient(): <?php echo $_SERVER['SERVER_ADDR'];  ?></title>
                ...
                <table border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        <td valign="top">                            
                            ...
                            <select name="server">
                                <option value="live" <?php echo $searchServer != 'test' ? 'selected' : ''; ?>>Live</option>
                                <option value="test" <?php echo $searchServer == 'test' ? 'selected' : ''; ?>>Test</option>
                            </select>
        ...
        ?>
            <table class="resultsTable">
                ...
                <?php foreach($result->data as $value) { 
                        ...
                        $rows = max($opetamisi, $oppimisi + $oppimisi2);
                        $rowSpan = '';
                        if ($rows > 1) {
                        ...
                        for ($i = 0; $i < $span; $i++) {
                            if (is_array($value->opetamine)) {
                                $opetamine =@ $value->opetamine[$i];    
                            } else if ($i == 0 && $value->opetamine) {
                                $opetamine =@ $value->opetamine;
                            } else {
                                $opetamine = array();
                            }
                            ...
                            if (is_array(@$value->oppimine_yld))
                                $oppimine_list[] = $value->oppimine_yld[$i];
                            else if($i == 0 && @$value->oppimine_yld != null)
                                $oppimine_list[] = @$value->oppimine_yld;
                            else {
                                // $oppimine_list[] = array();
                            }
                            
                            if (is_array(@$value->oppimine_korg))
                                $oppimine_list[] = $value->oppimine_korg[$i];
                            else if($i == 0 && @$value->oppimine_korg != null)
                                $oppimine_list[] = @$value->oppimine_korg;
                            else {
                                // $oppimine_list[] = array();
                            }
                    ...
                    ?>                        
                        ...
                        <?php } ?>
                            ...
                            <?php if (count($oppimine_list) > 0) {
                                foreach ($oppimine_list as $row => $oppimine) {
                                    if ($row) {
                            ?>
                        </tr>
                        <tr>
