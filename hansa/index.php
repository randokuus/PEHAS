...
if (defined("PANGALINK_MOCK")) {
    require_once(SITE_PATH . "/class/Pangalink/Hansa_Mock.php");
} else {
    require_once(SITE_PATH . "/class/Pangalink/Hansa.php");
}
// ##############################################################
// init main variables
...
if ($username) {
    ...
    $url = SITE_URL . "/hansa/id/" . $id;
    $pangalink = new Pangalink_Hansa();
    ...
    $error = $vkInfo ? 0 : 3;
    if (!$error && !$usr->setHansaSession($id, $vkInfo)) {
        $error = $usr->errorCode;
    }
...
// Hansanet authorization should be enabled
header('Location: ' . SITE_URL . '/?auth_type=3&auth_error=1&error_code=' . $error);
