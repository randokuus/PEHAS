<?php
define('MAIN_SITE_URL', 'https://www.minukool.ee/khs');
/**
...
if (!$error) {
    ...
    redirect(MAIN_SITE_URL . "/?nocache=true");
} else {
    // ID-card authorization should be enabled
    log_id_action("FAIL:" . print_r($_SERVER, true));
    header('Location: ' . MAIN_SITE_URL . '/?auth_type=2&auth_error=1&error_code=' . $error);
}