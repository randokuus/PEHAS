...
if (defined("PANGALINK_MOCK")) {
    require_once(SITE_PATH . "/class/Pangalink/Lhv_Mock.php");
} else {
    require_once(SITE_PATH . "/class/Pangalink/Lhv.php");
}
// ##############################################################
// init main variables
...
if ($username) {
    ...
    $url = SITE_URL . "/lhv/id/" . $id;
    $pangalink = new Pangalink_Lhv();
    ...
    $error = $vkInfo ? 0 : 8;
    if (!$error && !$usr->setLhvSession($id, $vkInfo)) {
        $error = $usr->errorCode;
    }
...
// Lhvpank authorization should be enabled
header('Location: ' . SITE_URL . '/?auth_type=8&auth_error=1&error_code=' . $error);
