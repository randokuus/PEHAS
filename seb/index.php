...
if (defined("PANGALINK_MOCK")) {
    require_once(SITE_PATH . "/class/Pangalink/Seb_Mock.php");
} else {
    require_once(SITE_PATH . "/class/Pangalink/Seb.php");
}
// ##############################################################
// init main variables
...
if ($username) {
    ...
    $url = SITE_URL . "/seb/id/" . $id;
    $pangalink = new Pangalink_Seb();
    ...
    $error = $vkInfo ? 0 : 4;
    if (!$error && !$usr->setSebSession($id, $vkInfo)) {
        $error = $usr->errorCode;
    }
...
// SEB authorization should be enabled
header('Location: ' . SITE_URL . '/?auth_type=4&auth_error=1&error_code=' . $error);
